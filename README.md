...
3. **📦 Order fulfilment** — confirmed orders update stock, notify you, send the
   customer a confirmation, and move the order through statuses.
...
│ post to:        │      │ reply on WhatsApp → │      │ email you +       │
│ WA·IG·FB·TikTok │      │ capture order       │      │ confirm customer  │
└─────────────────┘      └─────────────────────┘      └───────────────────┘
```
...
| [`n8n/3-order-fulfilment.json`](n8n/3-order-fulfilment.json) | Stock update + notifications |
...
TotalPrice, Currency, Status, FulfilmentNotes, PaymentStatus`
