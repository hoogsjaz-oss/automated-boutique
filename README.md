...
3. **📦 Order fulfilment + payment** — confirmed orders update stock, generate a
   **Yoco payment link**, send the customer a WhatsApp confirmation with the link,
   notify you, and move the order through statuses.
...
│ post to:        │      │ reply on WhatsApp → │      │ Yoco pay link →   │
...
| [`n8n/3-order-fulfilment.json`](n8n/3-order-fulfilment.json) | Stock update + Yoco payment link + notifications |
...
7. **[Payment setup](docs/07-payment-setup.md)** — Yoco key for the order pay link (or swap to Stripe/PayFast).
...
TotalPrice, Currency, Status, FulfilmentNotes, PaymentStatus, PaymentLink`
